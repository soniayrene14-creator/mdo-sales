...
import 'package:flutter_pos/domain/usecases/params/login_params.dart';
...
void main() {
  ...
  group('LoginUsecase', () {
    ...
    test('should return user from repository on successful login', () async {
      ...
      when(mockAuthRepository.login('user', 'pass')).thenAnswer((_) async => result);

      final response = await usecase.call(const LoginParams(email: 'user', password: 'pass'));

      expect(response, result);
      verify(mockAuthRepository.login('user', 'pass'));
      verifyNoMoreInteractions(mockAuthRepository);
    });

    test('should return failure from repository', () async {
      final result = Result<UserEntity>.failure(error: 'Login failed');
