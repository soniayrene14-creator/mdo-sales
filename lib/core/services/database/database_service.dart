...
class DatabaseService {
  ...
  Future<void> init() async {
    ...
    // Best-effort migration for installs whose Product table predates these
    // columns. CREATE TABLE IF NOT EXISTS above is a no-op on those installs.
    for (final statement in DatabaseConfig.productTableMigrationColumns) {
      try {
    ...
    for (final statement in DatabaseConfig.transactionTableMigrationColumns) {
      try {
        await database.execute(statement);
      } catch (_) {
        // Column already exists, ignore.
