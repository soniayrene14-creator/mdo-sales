...
class TransactionLocalDatasourceImpl extends TransactionDatasource {
        ...
        if (transaction.orderedProducts?.isNotEmpty ?? false) {
          ...
          for (var orderedProduct in transaction.orderedProducts!) {
            ...
            batch.insert(
              ...
              orderedProduct.toJson()..remove('unit_price'),
              conflictAlgorithm: ConflictAlgorithm.replace,
            );
  ...
  /// Reassigns a locally-cached transaction's id once the server has
  /// assigned its real id, so future syncs recognize it instead of
  /// re-creating it. Also updates the ordered products that reference it.
